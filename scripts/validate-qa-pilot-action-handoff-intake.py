#!/usr/bin/env python3
"""
QA Workbench Action Handoff Intake Validator.

HI Rules:
  HI-1: action_state valid enum  HI-2: advisory_only True  HI-3: custody qa-pilot-local
  HI-4: librarian_impact none   HI-5: authority_disclaimer match
  HI-6: No execution/authorization/seal/approval/verification/closure/mutation fields
  HI-7: Rationale no authority claims  HI-8: No registry/RCR/SRS fields
"""
...
def validate_all_rules(record):
    results = []
    if record.get("action_state") not in VALID_STATES: results.append(("HI-1", f"action_state must be one of {VALID_STATES}"))
    if record.get("advisory_only") is not True: results.append(("HI-2", "advisory_only must be True"))
    if record.get("custody", "") != "qa-pilot-local": results.append(("HI-3", "custody must be qa-pilot-local"))
    if record.get("librarian_impact", "") != "none": results.append(("HI-4", "librarian_impact must be 'none'"))
    if record.get("authority_disclaimer", "") != DISCLAIMER: results.append(("HI-5", "authority_disclaimer mismatch"))
    forbidden = ["executed_","execution_result","authorizes_execution","seal_","sealed",
                 ...
                 "mutates_intake","mutates_summary","mutates_receipt","mutates_packet","mutates_export"]
    for key in record:
        kl = key.lower()
        for p in forbidden:
            if p in kl: results.append(("HI-6", f"forbidden field '{key}' claims {p.replace('_',' ')}"))
    ...
    for kw in ["executed","authorizes","seal","approved","verified","closed","defect accepted"]:
        if kw in rationale: results.append(("HI-7", f"rationale contains authority-claiming term '{kw}'"))
    for key in record:
        kl = key.lower()
        if any(kw in kl for kw in ["registry","rcr_","srs_"]): results.append(("HI-8", f"intake carries registry/RCR/SRS field '{key}'"))
    return results

def cmd_fixture(args):
    directory = args.directory or os.path.join(PROJECT_ROOT,"docs","examples","qa-pilot-action-handoff-intake")
    schema = load_schema()
    ...
    for fpath in json_files:
        ...
        if is_valid:
            ...
            else: print(f"  {fname}: FAIL (expected PASS)"); [print(f"    [{c}] {d}") for c,d in all_issues]; failed += 1
        elif is_invalid:
            if all_issues: print(f"  {fname}: PASS (rejected as expected)"); [print(f"    [{c}] {d}") for c,d in all_issues[:3]]; passed += 1
...
def cmd_validate(args):
    ...
    for fpath in args.json_files:
        ...
        rule_issues = validate_all_rules(record)
        all_issues = schema_issues + rule_issues
        hid = record.get("handoff_id", fpath)
        if not all_issues: print(f"VALID: {hid}")
        else: print(f"INVALID: {hid}"); [print(f"  [{c}] {d}") for c,d in all_issues]

def cmd_live(args):
    schema = load_schema()
    if not os.path.exists(STORE_INDEX): print("No live handoff intake store found."); return
    with open(STORE_INDEX) as f: index = json.load(f)
    records = index.get("records", [])
    if not records: print("No live handoff intake records found."); return
    passed = failed = 0
    for hid in records:
        path = os.path.join(STORE_DIR, f"{hid}.json")
        if not os.path.exists(path): print(f"  {hid}: MISSING"); failed += 1; continue
        with open(path) as f: record = json.load(f)
        schema_ok, schema_issues = validate_schema(record, schema)
        rule_issues = validate_all_rules(record)
        all_issues = schema_issues + rule_issues
        if not all_issues: print(f"  {hid}: PASS"); passed += 1
        else: print(f"  {hid}: FAIL"); [print(f"    [{c}] {d}") for c,d in all_issues]; failed += 1
    print(f"\nLive handoff intake validation: {passed} pass, {failed} fail")
    if failed > 0: sys.exit(1)

def main():
    parser = argparse.ArgumentParser(description="QA Workbench Action Handoff Intake Validator")
